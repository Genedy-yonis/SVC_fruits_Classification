...
print(os.listdir(r"D:\ds\kaggle\fruits"))
dim = 10
...
def getYourFruits(fruits, data_type, print_n=False, k_fold=False):
    ...
    val = ['Training', 'Test']
    if not k_fold:
        path = r"D:\ds\kaggle\fruits/fruits-360/" + data_type + "/"
        ...
        for v in val:
            path = r"D:\ds\kaggle\fruits/fruits-360/" + v + "/"
            for i, f in enumerate(fruits):
                p = path + f
                j = 0
                for image_path in glob.glob(os.path.join(p, "*.jpg")):
...
def getAllFruits():
    fruits = []
    for fruit_path in glob.glob(r"D:\ds\kaggle\fruits/fruits-360/Training/*"):
